...
export default function ScamQuiz({ language, onEarnXP }) {
  ...
  return (
    <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 pt-4 pb-12 animate-fadeIn">
      {/* Quiz Header & Stats */}
      <div className="bg-white rounded-2xl shadow-lg border border-slate-200/80 p-6 sm:p-8 mb-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
          <div>
            <div className="inline-flex items-center gap-2 text-trust-700 font-extrabold text-lg sm:text-xl mb-1">
          ...
          <div className="flex items-center gap-4 bg-slate-50 border border-slate-200/80 px-4 py-2 rounded-xl text-xs sm:text-sm font-bold text-slate-700 shadow-inner">
            <div className="flex items-center gap-1.5">
              <span className="text-slate-400">{labels.scoreLabel[language] || "Score:"}</span>
              <span className="text-trust-600 font-extrabold text-base">{score} / {answeredCount}</span>
            </div>
        ...
        <div className="mt-6">
          ...
          <div className="p-5 sm:p-6 rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-800 text-base sm:text-lg font-medium leading-relaxed shadow-sm">
            "{currentScenario.text}"
          </div>

          {/* Question & Answer Buttons */}
          <div className="mt-8 space-y-4">
            <h3 className="text-center font-bold text-slate-800 text-base sm:text-lg flex items-center justify-center gap-2">
              <HelpCircle className="w-5 h-5 text-trust-600" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              <button
                ...
                className={`p-4 rounded-xl font-bold text-sm sm:text-base border-2 transition-all duration-200 flex items-center justify-center gap-2.5 shadow-sm ${
                  selectedAnswer === null
                    ? 'bg-rose-50 border-rose-200 text-rose-700 hover:bg-rose-100 hover:border-rose-300 hover:shadow-md'
              ...
              <button
                ...
                className={`p-4 rounded-xl font-bold text-sm sm:text-base border-2 transition-all duration-200 flex items-center justify-center gap-2.5 shadow-sm ${
                  selectedAnswer === null
                    ? 'bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100 hover:border-emerald-300 hover:shadow-md'
            ...
            <div className={`mt-8 p-5 rounded-xl border-2 animate-fadeIn transition-all ${
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 font-bold text-base">
                    {isCorrect ? (
                      <>
                        ...
                        <span className="ml-1 px-2 py-0.5 rounded-full bg-emerald-200 text-emerald-800 text-xs">+100 XP</span>
                      </>
                    ) : (
                      <>
                        <XCircle className="w-5 h-5 text-amber-600" />
                        <span>{labels.wrongMsg[language] || labels.wrongMsg.en}</span>
                        <span className="ml-1 px-2 py-0.5 rounded-full bg-amber-200 text-amber-800 text-xs">+25 XP</span>
                      </>
                    )}
                  </div>
                ...
                <button
                  type="button"
                  onClick={handleNext}
                  className="flex-shrink-0 px-6 py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs sm:text-sm shadow-md transition-all flex items-center justify-center gap-2 active:scale-[0.99]"
                >
